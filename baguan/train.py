...
from baguan.modules import BaguanV2Module
...
def main(args=Args()):
    ...
    proj_name = 'final_version_6h_stage1'

    callbacks = [
        ModelCheckpoint(
            dirpath=f'/jupyter/BaguanV2/checkpoint/{proj_name}',
            save_top_k=100,
            save_last=True,
            monitor='train/loss',
    ...
    logger = WandbLogger(
        name=proj_name,
        save_dir=f"./logs/baguan_v2/{proj_name}",
        project='baguanv2',
    ...
    trainer.fit(
        model, datamodule,
        # ckpt_path="/jupyter/BaguanV2/checkpoint/patch6_hid1536_head16_window20_swinv2_singleframe/epoch_epoch=009-v9.ckpt"
    )
